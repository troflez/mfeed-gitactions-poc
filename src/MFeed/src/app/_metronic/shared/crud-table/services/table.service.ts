...
import { ITableState, TableResponseModel, TableState } from '../models/table.model';
...
export abstract class TableService<T> {
  ...
  private _tableState$ = new BehaviorSubject<ITableState>(new TableState());
  ...
  protected fileUploadHeader(): object{
  ...
  create(item: BaseModel): Observable<BaseModel> {
    ...
    //console.log(item);
    return this.http.post<BaseModel>(url, item, this.header()).pipe(
      catchError(err => {
  ...
  find(tableState: ITableState): Observable<TableResponseModel<T>> {
    ...
    this._errorMessage.next('');
    return this.http.post<TableResponseModel<T>>(url, tableState, this.header()).pipe(
      catchError(err => {
        this._errorMessage.next(err);
  ...
  getItemById(id: number): Observable<BaseModel> {
    ...
    this._isLoading$.next(true);
    this._errorMessage.next('');
    return this.http.get<BaseModel>(url, this.header()).pipe(
      catchError(err => {
        this._errorMessage.next(err);
  ...
  update(item: BaseModel): Observable<any> {
    ...
    this._isLoading$.next(true);
    this._errorMessage.next('');
    return this.http.put(url, item, this.header()).pipe(
      catchError(err => {
        this._errorMessage.next(err);
